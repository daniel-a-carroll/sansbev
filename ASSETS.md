...
| `brand-board` | have | 1536×1024 | 3:2 | PNG | Wholesale page, full-bleed under the intro | FULFILLED. The three cans with the brand lockup and positioning copy. Lives on the WHOLESALE page, not the product page: it is a poster, carrying its own logo, headline and tagline, so nesting it in a page that already has all three read as a page within a page, and it duplicated the product page's own three cans. On the trade page nothing competes with it and it does the job it was made for. TWO THINGS TO KNOW. (1) It carries claim copy in the pixels, so it is transcribed into imageClaims in src/data/claims.ts and the build lint scans that transcript; if you replace this image, update the transcript in the same commit. (2) It is rendered full-bleed, so on a display wider than 1536px it upscales slightly. A 2400px+ export would fix that. |
...
| Slot | Status | Dimensions | Ratio | Format | Appears | What the shot should contain |
|---|---|---|---|---|---|---|
| `flavor-cranberry-can-front` | have | 400×1010 | 40:101 | PNG | Flavor page hero, product index card (cranberry) | FULFILLED by the cutouts in assets/. 400x1010 PNG with a real alpha channel, which is why cans can now sit directly on flavor color fields. Any replacement must keep transparency: an opaque render reintroduces a pale rectangle on every colored band. |
| `flavor-cranberry-can-angle` | NEEDED | 400×1010 | 40:101 | PNG | Flavor page secondary (cranberry) | Same can, three-quarter angle, transparent background, matching the front cutout's framing and lighting. |
...
| `flavor-lime-can-front` | have | 400×1010 | 40:101 | PNG | Flavor page hero, product index card (lime) | FULFILLED by the cutouts in assets/. 400x1010 PNG with a real alpha channel, which is why cans can now sit directly on flavor color fields. Any replacement must keep transparency: an opaque render reintroduces a pale rectangle on every colored band. |
| `flavor-lime-can-angle` | NEEDED | 400×1010 | 40:101 | PNG | Flavor page secondary (lime) | Same can, three-quarter angle, transparent background, matching the front cutout's framing and lighting. |
...
| `flavor-pineapple-can-front` | have | 400×1010 | 40:101 | PNG | Flavor page hero, product index card (pineapple) | FULFILLED by the cutouts in assets/. 400x1010 PNG with a real alpha channel, which is why cans can now sit directly on flavor color fields. Any replacement must keep transparency: an opaque render reintroduces a pale rectangle on every colored band. |
| `flavor-pineapple-can-angle` | NEEDED | 400×1010 | 40:101 | PNG | Flavor page secondary (pineapple) | Same can, three-quarter angle, transparent background, matching the front cutout's framing and lighting. |
