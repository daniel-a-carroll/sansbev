...
| `og-default` | NEEDED | 1200×630 | 40:21 | PNG | Open Graph / social preview for any page without its own image | The INGREDIENT PANEL, per the landing page spec: the panel is the pitch, so it is what should appear when the page is shared. A lifestyle shot here would undercut the whole argument. Until this exists the site emits no OG image, which is better than the wrong one. |
